...
class Server{
		...
		/*if(file_exists($path . "tiles.yml")){
			...
			foreach($tiles->getAll() as $tile){
				...
				foreach($tile as $index => $data){
					switch($index){
						case "Items":
							$tag = new EnumTag("Items", []);
							$tag->setTagType(NBT::TAG_Compound);
							foreach($data as $slot => $fields){
								$tag[(int) $slot] = new CompoundTag(false, array(
									"Count" => new ByteTag("Count", $fields["Count"]),
									"Slot" => new ShortTag("Slot", $fields["Slot"]),
									"Damage" => new ShortTag("Damage", $fields["Damage"]),
									"id" => new StringTag("id", $fields["id"])
								));
							}
							$nbt["Items"] = $tag;
							break;
						...
						case "Text3":
						case "Text4":
							$nbt[$index] = new StringTag($index, $data);
							break;
						...
						case "pairx":
						case "pairz":
							$nbt[$index] = new IntTag($index, $data);
							break;

						case "BurnTime":
						case "CookTime":
						case "MaxTime":
							$nbt[$index] = new ShortTag($index, $data);
							break;
					}
				}
				switch($tile["id"]){
	...
	public function exceptionHandler($e, $trace = null){
		if($e === null){
			return;
		}
